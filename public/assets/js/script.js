var vue = new Vue({
    ...
    mounted() {
        ...
        if (document.getElementById("sel-opt")) {
            ...
            let banner = document.querySelector('.banner-face')
            console.log(banner.textContent)
            if (banner.textContent == 'ru') {
                spanText.textContent = 'Вид деятельности';
            } 
            if (banner.textContent == 'en') {
                spanText.textContent = 'Type of activity';
            } 
            if (banner.textContent == 'kz') {
                spanText.textContent = 'Қызмет түрі';
            }
            ...
            for (var i = 0; i < option.length; i++) {
                option[i].addEventListener('click', function() {
                        ...
                        vue.toggleSelectValue(this.getAttribute('data_value'))
                });
            }
